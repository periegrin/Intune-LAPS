...
The Intune assignment was not the issue. The endpoint was receiving LAPS-related configuration from another management path.

This was an important finding because it demonstrated that policy targeting needs to be evaluated together with the existing endpoint management configuration.
...
The experience also reinforced the importance of checking existing Group Policy and other endpoint management mechanisms before introducing a new Intune configuration.
...
The verification was performed using the management interfaces available to the IT team.

No credentials or sensitive tenant information are included in this repository.
...
The problem was not discovered simply by looking at the Intune policy. It was discovered by comparing the expected device scope with what was actually happening on the endpoints and then investigating the existing management configuration.

---

## Security and Privacy

This repository is intended to document the implementation approach and lessons learned without exposing information that could be used to identify or compromise the environment.
...
The repository should therefore be treated as a technical case study rather than a copy of the organization's production configuration.

---

## Project Status

This project documents the implementation approach, deployment experience, troubleshooting, verification process, and lessons learned from implementing Windows LAPS with Microsoft Intune and Microsoft Entra ID.

Additional documentation and sanitized examples may be added as the project develops.
